...
func TestIsAgari(t *testing.T) {
	...
	for _, humanTiles := range []string{
		"119m 19p 19s 1234567z", // 國士無雙自行判斷
		"1133555599m 1122s",
		"1122m",
		"8888p",
		"7777z",
...
func TestDivideTiles34(t *testing.T) {
	...
	const otherDivideResult = "國士 or 未和牌"
	...
	assert.Equal("[七對子]", divideTiles("11223344556677z"))
	assert.Equal("[七對子]", divideTiles("223344m 11335577s"))
	assert.Equal("[99s 111s 123s 456s 789s][九蓮寶燈][一氣通貫]", divideTiles("11112345678999s"))
	assert.Equal("[22s 111s 999s 345s 678s][九蓮寶燈]", divideTiles("11122345678999s"))
	assert.Equal("[11s 999s 123s 345s 678s][九蓮寶燈]", divideTiles("11123345678999s"))
	assert.Equal("[99s 111s 234s 456s 789s][九蓮寶燈]", divideTiles("11123445678999s"))
	assert.Equal("[55s 111s 999s 234s 678s][九蓮寶燈]", divideTiles("11123455678999s"))
	assert.Equal("[11s 999s 123s 456s 789s][九蓮寶燈][一氣通貫]", divideTiles("11123456789999s"))
	assert.Equal("[44s 123m 456m 789m 123s][一氣通貫]", divideTiles("123456789m 12344s"))
	assert.Equal("[11m 123p 456p 789p][一氣通貫]", divideTiles("11m 123456789p"))
	assert.Equal("[11p 123p 456p 789p][一氣通貫]", divideTiles("11123456789p"))
	assert.Equal("[11z 123m 123m 123p 123p][兩盃口]", divideTiles("112233m 112233p 11z"))
	assert.Equal("[22m 345m 345m 234p 234s][一盃口], [55m 234m 234m 234p 234s][一盃口]", divideTiles("22334455m 234s 234p"))
	assert.Equal("[11z 111m 222m 333m 234s], [11z 123m 123m 123m 234s][一盃口]", divideTiles("111222333m 234s 11z"))
