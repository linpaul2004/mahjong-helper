...
func TestCalculateShantenAndWaits13(t *testing.T) {
	...
	// closed
	assert.Equal("聽牌 3 進張 [7z]", toString(CalculateShantenAndWaits13(MustStrToTiles34("1122334455667z"), nil)))
	assert.Equal("聽牌 4 進張 [4s]", toString(CalculateShantenAndWaits13(MustStrToTiles34("123456789m 1135s"), nil)))
	assert.Equal("聽牌 8 進張 [25s]", toString(CalculateShantenAndWaits13(MustStrToTiles34("123456789m 1134s"), nil)))
	assert.Equal("一向聽 61 進張 [12345678m 47p 12345678s]", toString(CalculateShantenAndWaits13(MustStrToTiles34("3456m 3456s 44456p"), nil)))
	assert.Equal("兩向聽 12 進張 [1234z]", toString(CalculateShantenAndWaits13(MustStrToTiles34("123456789m 1234z"), nil)))
	assert.Equal("三向聽 32 進張 [46m 2468p 24s]", toString(CalculateShantenAndWaits13(MustStrToTiles34("11357m 13579p 135s"), nil)))

	// open
	assert.Equal("聽牌 3 進張 [5p]", toString(CalculateShantenAndWaits13(MustStrToTiles34("5p"), nil)))
	assert.Equal("聽牌 6 進張 [14p]", toString(CalculateShantenAndWaits13(MustStrToTiles34("1234p"), nil)))
	assert.Equal("一向聽 132 進張 [12346789m 123456789p 123456789s 1234567z]", toString(CalculateShantenAndWaits13(MustStrToTiles34("5555m"), nil)))
	assert.Equal("兩向聽 12 進張 [1234z]", toString(CalculateShantenAndWaits13(MustStrToTiles34("1234z"), nil)))
}
