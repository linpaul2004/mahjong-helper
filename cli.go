...
func printAccountInfo(accountID int) {
	fmt.Printf("您的帳號 ID 為 ")
	color.New(color.FgHiGreen).Printf("%d", accountID)
	fmt.Printf("，該數字為雀魂服務器帳號數據庫中的 ID，該值越小表示您的註冊時間越早\n")
}

//

func (p *playerInfo) printDiscards() {
	// TODO: 高亮不合理的捨牌或危險捨牌，如
	// - 一開始就切中張
	// - 開始切中張後，手切了幺九牌（也有可能是有人碰了牌，比如 133m 有人碰了 2m）
	// - 切了 dora，提醒一下
	// - 切了赤寶牌
	// - 有人立直的情況下，多次切出危險度高的牌（有可能是對方讀準了牌，或者對方手裏的牌與牌河加起來産生了安牌）
	// - 其餘可以參考貼吧的《魔神之眼》翻譯 https://tieba.baidu.com/p/3311909701
	//      舉個簡單的例子,如果出現手切了一個對子的情況的話那麼基本上就不可能是七對子。
	//      如果對方早巡手切了一個兩面搭子的話，那麼就可以推理出他在做染手或者牌型是對子型，如果他立直或者鳴牌的話，也比較容易讀出他的手牌。
	// https://tieba.baidu.com/p/3311909701
	//      鳴牌之後和終盤的手切牌要盡量記下來，別人手切之前的安牌應該先切掉
	// https://tieba.baidu.com/p/3372239806
	//      吃牌時候打出來的牌的顔色是危險的；碰之後全部的牌都是危險的
	...
	for i, disTile := range p.discardTiles {
		...
		if disTile >= 0 { // 手切
			...
			if disTile >= 27 {
				tile = util.MahjongU[disTile] // 關注字牌的手切
			}
			if p.isNaki { // 副露
				fgColor = getOtherDiscardAlertColor(disTile) // 高亮中張手切
				if util.InInts(i, p.meldDiscardsAt) {
					bgColor = color.BgWhite // 鳴牌時切的那張牌要背景高亮
...
type riskInfo struct {
	...
	// 該玩家的安牌
	// 若該玩家有槓操作，把槓的那張牌也算作安牌，這有助於判斷筋壁危險度
	safeTiles34 []bool
...
func (l riskInfoList) printWithHands(hands []int, leftCounts []int) {
	...
	for i := len(l) - 1; i >= 1; i-- {
		...
		if len(l[i].riskTable) > 0 && (debugMode || tenpaiRate > minShownTenpaiRate) {
			...
			//if debugMode {
			//fmt.Printf("(%d*%2.2f%%聽牌率)", int(l[i]._ronPoint), l[i].tenpaiRate)
			//}
	...
	if shanten >= 1 {
		...
		if shanten >= 2 {
			...
			if showAgariAboveShanten1 {
				fmt.Printf("（%.2f%% 參考和率）", result13.AvgAgariRate)
			}
		}
		if showScore {
			...
			//	mixedScore /= 4
			//}
			fmt.Printf("（%.2f 綜合分）", mixedScore)
		}
	} else { // shanten == 0
		fmt.Printf("%5.2f%% 參考和率", result13.AvgAgariRate)
	}
...
/*
4[ 4.56] 切 8餅 => 44.50% 參考和率[ 4 改良] [7p 7s] [默聽2000] [三色] [振聽]

4[ 4.56] 切 8餅 => 0.00% 參考和率[ 4 改良] [7p 7s] [無役]

31[33.58] 切7索 =>  5.23聽牌數 [19.21速度] [16改良] [6789p 56789s] [局收支3120] [可能振聽]

48[50.64] 切5餅 => 24.25一向聽 [12改良] [123456789p 56789s]

31[33.62] 77索碰,切5餅 => 5.48聽牌數 [15 改良] [123456789p]
...
func (r *analysisResult) printWaitsWithImproves13_oneRow() {
	...
	if shanten >= 1 {
		...
		if result13.FuritenRate == 1 || result13.IsPartWait {
			color.New(color.FgHiRed).Printf("%5.2f%% 參考和率", result13.AvgAgariRate)
		} else {
			fmt.Printf("%5.2f%% 參考和率", result13.AvgAgariRate)
		}
	}
...
func printResults14WithRisk(results14 util.Hand14AnalysisResultList, mixedRiskTable riskTable) {
	...
	// FIXME: 選擇很多時如何精簡何切選項？
	//const maxShown = 10
	//if len(results14) > maxShown { // 限製輸出數量
